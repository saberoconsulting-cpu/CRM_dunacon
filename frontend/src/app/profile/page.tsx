  ...
  return (
    <Layout title="Mi perfil">
      ...
      <div className="w-full space-y-5">
        ...
        <div className="card p-5 sm:p-6 flex flex-col sm:flex-row items-center sm:items-start gap-5 text-center sm:text-left">
          ...
          <div className="relative shrink-0">
            <button
              ...
              className="overflow-hidden rounded-full border-2 border-brand-200 block transition-opacity hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2"
              style={{ width: 96, height: 96 }}
            >
              {me.photoUrl ? (
                <img src={me.photoUrl} alt="" className="w-full h-full object-cover" />
              ) : (
                <span className="w-full h-full flex items-center justify-center bg-brand-600 text-white text-3xl font-bold uppercase">
                  {me.name?.charAt(0) || '?'}
                </span>
              )}
            </button>
            ...
            {avatarBusy && (
              <span className="absolute inset-0 rounded-full bg-black/40 text-white text-xs font-medium flex items-center justify-center backdrop-blur-[1px]">
                Subiendo…
              </span>
            )}
            ...
            <button
              type="button"
              onClick={() => fileRef.current?.click()}
              className="absolute bottom-0 right-0 bg-brand-600 hover:bg-brand-700 text-white rounded-full w-8 h-8 grid place-items-center shadow-md transition-transform active:scale-95"
              title="Cambiar foto"
            >
              <FiEdit2 style={{ fontSize: 14 }} />
            </button>
          ...
          <div className="flex-1 w-full min-w-0">
            <div className="text-xl font-bold text-slate-900 truncate">{me.name}</div>
          ...
          {(me.phone?.trim() || me.whatsapp?.trim()) && (
            <div className="w-full sm:w-auto text-sm bg-canvas rounded-xl p-3.5 space-y-2 border border-slate-100 shrink-0">
              {me.phone?.trim() && (
                <div className="flex items-center justify-center sm:justify-start gap-2 text-slate-700">
                  ...
                  <a className="hover:underline hover:text-brand-600 font-medium transition-colors" href={`tel:${me.phone}`}>
                    {me.phone}
                  </a>
                </div>
              )}
              {me.whatsapp?.trim() && (
                <div className="flex items-center justify-center sm:justify-start gap-2 text-slate-700">
                  ...
                  <a
                    className="hover:underline hover:text-emerald-600 font-medium transition-colors"
                    href={`https://wa.me/${me.whatsapp.replace(/[^\d]/g, '')}`}
                    target="_blank"
                    rel="noreferrer"
                  >
                    {me.whatsapp}
